        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {features.map((feature, index) => (
            <RevealOnScroll key={index} delay={index * 0.1}>
                <div 
                className="bg-slate-50 p-6 rounded-sm border border-slate-200 hover:border-electric transition-all duration-300 hover:transform hover:-translate-y-1 group h-full"
                >
                <div className="mb-4 bg-white w-16 h-16 rounded-full flex items-center justify-center border border-slate-200 group-hover:border-electric transition-colors shadow-sm">
                    {feature.icon}
                </div>
                <h3 className="text-xl font-bold text-slate-900 mb-3 group-hover:text-electric transition-colors">
                    {feature.title}
                </h3>
